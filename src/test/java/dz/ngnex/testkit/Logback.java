...
public class Logback extends BasicConfigurator {
  ...
  public void configure(final LoggerContext lc) {
    ...
    lc.getLogger("ROOT").setLevel(INFO);

    // App logging
    //  - use warn for transaction rollbacks
    //  - use info for state logging
    //  - use debug for transaction management logging
    //  - use trace for resource management logging
    lc.getLogger("dz.ngnex").setLevel(INFO);
    ...
    // Hibernate logging
    lc.getLogger("org.hibernate").setLevel(ERROR);
    lc.getLogger("org.hibernate.type").setLevel(ERROR);
