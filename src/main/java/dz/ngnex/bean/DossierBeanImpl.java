...
public class DossierBeanImpl implements DossierBean {
    ...
    @Override
    public DossierContentEntity add(BinaryContent file, String uploader) throws IntegrityException {
        if (file.getSize() > MAX_FILE_SIZE)
            throw new IntegrityException("file too big to be persisted", "fileTooBig");
        ...
        String dossierFileName = "~" + file.getName();
        if (!isNameUnique(dossierFileName))
            throw new IntegrityException("attachment already is included in the dossier: " + dossierFileName + ", uploader: " + uploader, "alreadyAddedToDossier");
