...
//        .setHint(Hints.FETCH_GRAPH, contractGraph)
    ...
    @Override
    public Integer getTutorials(EntityReference<? extends BasicPrincipalEntity> ref) {
        if (ref == null)
            return null;
        //noinspection JpaQlInspection
        List<Integer> result = em.createQuery("select p.tutorials from " + ref.getType().getSimpleName() + " p where p.id = :id", Integer.class)
                .setParameter("id", ref.getId())
                .setMaxResults(1)
                .getResultList();
        return DatabaseEntity.findFirst(result);
    }

    @Override
    public void setTutorial(EntityReference<? extends BasicPrincipalEntity> ref, int tutorials) {
        //noinspection JpaQlInspection
        em.createQuery("update " + ref.getType().getSimpleName() + " p set p.tutorials = :tutorials where p.id = :id")
                .setParameter("id", ref.getId())
                .setParameter("tutorials", tutorials)
                .executeUpdate();
    }
