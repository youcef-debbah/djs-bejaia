 ...
 * <li>{@link #getExpireTime(HttpServletRequest, BinaryContent)}
 * <li>{@link #getContentType(HttpServletRequest, BinaryContent)}
 * <li>{@link #isAttachment(HttpServletRequest, String)}
 * <li>{@link #getAttachmentName(HttpServletRequest, BinaryContent)}
 * </ul>
 *
...
public abstract class FileServlet extends HttpServlet {
  ...
  public static final Long DEFAULT_EXPIRE_TIME_IN_SECONDS = TimeUnit.HOURS.toSeconds(12);
   ...
   * If this method returns <code>null</code>, then the
   ...
   */
  protected abstract BinaryContent getFile(HttpServletRequest request);
   ...
   * <p>
   * The default implementation returns 12 hours in seconds.
   *
   * @param request The involved HTTP servlet request.
   ...
   * <p>
   * The default implementation delegates {@link BinaryContent#getContentType()}.
   *
   * @param request The involved HTTP servlet request.
   * @param file    The involved file.
   * @return The content type associated with the given HTTP servlet request and file.
   */
  protected String getContentType(HttpServletRequest request, BinaryContent file) {
    return file.getContentType();
  }

  /**
   * Returns <code>true</code> if we must force a "Save As" dialog based on the given HTTP servlet request and content
   * type as obtained from {@link #getContentType(HttpServletRequest, BinaryContent)}.
   * <p>
   ...
   * <p>
   * The default implementation returns {@link BinaryContent#getName()}.
   *
   * @param request The involved HTTP servlet request.
   * @param file    The involved file.
   ...
   * @since 2.3
   */
  protected String getAttachmentName(HttpServletRequest request, BinaryContent file) {
    return file.getName();
  }
   ...
   */
  private Range parseRange(String range, int length) {
    int start = sublong(range, 0, range.indexOf('-'));
    int end = sublong(range, range.indexOf('-') + 1, range.length());
  ...
  private void writeContent(HttpServletResponse response, Resource resource, List<Range> ranges, String contentType) throws IOException {
    ...
    if (ranges.size() == 1) {
      Range range = ranges.get(0);
      output.write(resource.file.getContents(), range.start, range.length);
    } else {
  ...
  private static int sublong(String value, int beginIndex, int endIndex) {
    ...
    return substring.isEmpty() ? -1 : Integer.parseInt(substring);
  }
  ...
  private static class Resource {
    @NotNull
    private final BinaryContent file;
    @NotNull
    private final String eTag;
    private final int length;
    private final long lastModified;

    public Resource(@Nullable BinaryContent file) throws FileNotFoundException {
      if (file != null) {
        this.file = file;
        this.length = file.getSize();
        this.lastModified = file.getUploadTime();
        this.eTag = format(ETAG, encodeURL(file.getName()), lastModified);
      } else
